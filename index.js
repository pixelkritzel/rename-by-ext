...
const follows = filenames.filter(name => !name.startsWith('.') && name.endsWith(`.${followExtension}`));

if (leads.length == follows.length) {
  for(let i = 0; i < leads.length; i++) {
    let leadName = leads[i].split(`.${leadExtension}`)[0];
    let follower = follows[i];
    fs.rename(`${absolutePath}/${follower}`, `${absolutePath}/${leadName}.${followExtension}`);
  }
}